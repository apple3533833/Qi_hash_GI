...
def jacobian(sess, x, grads, target, X, nb_features, nb_classes, feed=None):
    ...
    feed_dict = {x: X}
    #feed_dict = {x: X,x_clean:X,training_BN:False}
    if feed is not None:
...
def jsma(sess, x, predictions, grads, sample, target, theta, gamma, clip_min,
    ...
    while (current != target and iteration < max_iters and
        ...
        grads_target, grads_others = jacobian(sess, x, grads, target,
                                              ...
                                              feed=feed)
        #print(grads_target, grads_others)
...
def jsma_batch(sess, x, pred, grads, X, x_clean,training_BN,theta, gamma, clip_min, clip_max,
    ...
    feed = {x:X,x_clean:X,training_BN:False}	
...
def jsma_symbolic(x, y_target, model, x_clean,training_BN,theta, gamma, clip_min, clip_max):
    """
    TensorFlow implementation of the JSMA (see https://arxiv.org/abs/1511.07528
    for details about the algorithm design choices).
    ...
    def condition(x_in, x_clean,training_BN,y_in, domain_in, i_in, cond_in):
    ...
    def body(x_in, x_clean,training_BN,y_in, domain_in, i_in, cond_in):

        #preds = model.get_probs(x_in)
        #print(x_in)
        preds=model.get_probs_BN(x_in,training=training_BN,is_CW=True)
        ...
        return x_out,x_clean,training_BN, y_in, domain_out, i_out, cond_out

    # Run loop to do JSMA
    x_adv, _, _, _, _ ,_,_= tf.while_loop(condition, body,
                                      [x, x_clean,training_BN,y_target, search_domain, 0, True],
...
class CarliniWagnerL2(object):
    ...
    def __init__(self, sess, model, x_clean,training_BN,batch_size, confidence,
        ...
        self.output = model.get_logits_BN(self.newimg,training=self.training_BN,restore_parameters=True)
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                _, l, l2s, scores, nimg = self.sess.run([self.train,
                                                         ...
                                                         self.l2dist,
                                                         self.output,
                                                         self.newimg],feed_dict={self.x_clean:batch,self.training_BN:training_BN})
        ...
        def doit(oimgs, labs, starts, valid, CONST):
            ...
            while CONST < self.LARGEST_CONST:
                ...
                for step in range(self.MAX_ITERATIONS):
                    ...
                    feed_dict={const: CONST,self.x_clean:imgs,self.training_BN:training_BN}
