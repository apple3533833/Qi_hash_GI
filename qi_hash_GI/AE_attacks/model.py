...
class Model(object):
    ...
    def get_layer_BN(self, x, layer,training,restore_parameters=False):
        """
        Expose the hidden features of a model given a layer name.
        :param x: A symbolic representation of the network input
        :param layer: The name of the hidden layer to return features at.
        ...
        """
        # Return the symbolic representation for this layer.
        output = self.fprop(x,training=training,restore_parameters=restore_parameters)
        try:
            requested = output[layer]
        except KeyError:
            raise NoSuchLayerError()
        return requested
    def get_layer_BN_hash_retraining(self, x, layer,training,restore_parameters=False,hash_retraining=False):
        """
        Expose the hidden features of a model given a layer name.
        :param x: A symbolic representation of the network input
        :param layer: The name of the hidden layer to return features at.
        ...
        """
        # Return the symbolic representation for this layer.
        output = self.fprop(x,training=training,restore_parameters=restore_parameters,hash_retraining=hash_retraining)
        try:
            requested = output[layer]
        except KeyError:
            raise NoSuchLayerError()
    ...
    def get_logits_BN(self, x,training,restore_parameters=False):
        """
        :param x: A symbolic representation of the network input
        :return: A symbolic representation of the output logits (i.e., the
                 values fed as inputs to the softmax layer).
        """
        return self.get_layer_BN(x, 'logits',training,restore_parameters)
    ...
    def get_probs_BN(self, x,training,restore_parameters=False):
        """
        :param x: A symbolic representation of the network input
        :return: A symbolic representation of the output probabilities (i.e.,
                the output values produced by the softmax layer).
        """
        try:
            return self.get_layer_BN(x, 'probs' ,training,restore_parameters)
        except NoSuchLayerError:
            import tensorflow as tf
            return tf.nn.softmax(self.get_logits(x))
  
    def get_probs_BN_hash_retraining(self, x,training,hash_retraining):
        """
        :param x: A symbolic representation of the network input
        :return: A symbolic representation of the output probabilities (i.e.,
                the output values produced by the softmax layer).
        """
        try:
            return self.get_layer_BN(x, 'probs' ,training,hash_retraining)
        except NoSuchLayerError:
            import tensorflow as tf
            return tf.nn.softmax(self.get_logits(x))
