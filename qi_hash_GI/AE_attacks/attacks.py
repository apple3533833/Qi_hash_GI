...
import AE_attacks.utils as utils
from AE_attacks.model import Model, CallableModelWrapper
...
class Attack(object):
    ...
    def __init__(self, model, back='tf', sess=None,training_BN=None,x_clean=None):
        """
    ...
    def generate_np(self, x_val, **kwargs):
        ...
        if self.training_BN is not None:
            feed_dict = {x: x_val,self.x_clean:x_val,self.training_BN:False}
        else:
            feed_dict = {x: x_val}
        #print(feed_dict)
    ...
    def get_or_guess_labels(self, x, kwargs):
        ...
        if 'y' in kwargs and 'y_target' in kwargs:
            ...
            preds =	self.model.get_probs_BN(x,training=self.training_BN,restore_parameters=True)
...
class FastGradientMethod(Attack):
    ...
    def __init__(self, model, back='tf', sess=None,x_clean=None,training_BN=None):
        """
    ...
    def generate(self, x, **kwargs):
        ...
        model_preds =self.model.get_probs_BN(x,training=self.training_BN,restore_parameters=True)
        return fgm(x, model_preds, y=labels, eps=self.eps,
    ...
    def generate(self, x, **kwargs):
        ...
        for i in range(self.nb_iter):
            FGM = FastGradientMethod(self.model, back=self.back,
                                     sess=self.sess,training_BN=self.training_BN)
...
class SaliencyMapMethod(Attack):
    ...
    def __init__(self, model, back='tf', sess=None,x_clean=None,training_BN=None):
        ...
        cleverhans.model.Model abstraction provided by CleverHans.
        """
        super(SaliencyMapMethod, self).__init__(model, back, sess,training_BN,x_clean)
        self.training_BN=training_BN
        self.x_clean=x_clean
        self.restore_parameters=True
        if not isinstance(self.model, Model):
            self.model = CallableModelWrapper(self.model, 'probs')
    ...
    def generate(self, x, **kwargs):
        ...
        if self.symbolic_impl:
            ...
            x_adv = jsma_symbolic(x, model=self.model, y_target=self.y_target,x_clean=self.x_clean,training_BN=self.training_BN,
            ...
            # JSMA-Z
            preds = self.model.get_logits_BN(x,training=self.training_BN,restore_parameters=True)
            # JSMA-F
            #preds = self.model.get_probs_BN(x,training=self.training_BN,restore_parameters=True)
            ...
            if self.y_target is not None:
                def jsma_wrap(x_val, y_target):
                    return jsma_batch(self.sess, x, preds, grads, x_val,self.x_clean,self.training_BN,
...
class CarliniWagnerL2(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        attack = CWL2(self.sess, self.model, self.x_clean,self.training_BN,self.batch_size,
...
class CarliniWagnerL0(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        #print(x)
        #print(labels)
        #print('y_target' in kwargs)				 
        attack = CWL0(self.sess, self.model, self.x_clean,self.training_BN,
                      self.confidence, 'y_target' in kwargs
                      )
